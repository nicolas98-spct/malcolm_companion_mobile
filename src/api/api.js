...
function withImages(items) {
  return items.map((item) => ({ ...item, image: localImages[item.imageKey] ?? localImages.logo }));
}
...
export async function getCharacters() { return withImages(await request('/personajes', charactersFallback).catch(() => charactersFallback)); }
export async function getCharacter(id) { return (await getCharacters()).find((item) => String(item.id) === String(id)) ?? withImages(charactersFallback)[0]; }
export async function getEpisodes() { return withImages(await request('/episodios', episodesFallback).catch(() => episodesFallback)); }
export async function getEpisode(id) { return (await getEpisodes()).find((item) => String(item.id) === String(id)) ?? withImages(episodesFallback)[0]; }
export async function getClips() { return withImages(await request('/clips', clipsFallback).catch(() => clipsFallback)); }
export async function getUser() { const user = await request('/usuario', userFallback).catch(() => userFallback); return { ...user, avatar: localImages[user.avatarKey] ?? localImages.avatarJuan }; }
