...
export default function CharactersScreen({ navigation }) {
  ...
  const list = useMemo(() => (data || []).filter((c) => c.role === chip && c.name.toLowerCase().includes(query.toLowerCase())), [data, chip, query]);
  if (loading) return <Loader />;
  return <ScrollView style={{ flex: 1, backgroundColor: '#f1f1f1' }} contentContainerStyle={{ paddingBottom: 24 }}><View style={{ backgroundColor: '#d71920', padding: 22, paddingTop: 58, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 }}><Text style={{ color: '#fff', fontSize: 36, fontWeight: '900' }}>Personajes</Text><TextInput placeholder="Buscar personaje" style={{ backgroundColor: '#fff', borderRadius: 999, padding: 14, marginTop: 14 }} value={query} onChangeText={setQuery} /></View><ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ padding: 16 }}>{chips.map((item) => <Pressable key={item} onPress={() => setChip(item)} style={{ backgroundColor: chip === item ? '#101010' : '#fff', borderRadius: 999, paddingVertical: 10, paddingHorizontal: 16, marginRight: 8 }}><Text style={{ color: chip === item ? '#fff' : '#111', fontWeight: '900' }}>{item}</Text></Pressable>)}</ScrollView><View style={{ paddingHorizontal: 16 }}>{list.length ? list.map((character) => <CharacterCard key={character.id} character={character} onPress={() => navigation.navigate('CharacterDetail', { id: character.id })} />) : <EmptyState />}</View></ScrollView>;
}
